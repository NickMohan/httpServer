...
public class Server{
	...
	public static void main(String[] args) throws IOException{
		...
		//Set up te executor pool for the multithreading
		Executor service = Executors.newCachedThreadPool();

		while(true){
			try{
				//Opens up the connection to the client
				Socket client = serverSock.accept();
				//Blacklist check
				if (isBlackListed(client.getLocalAddress())){
    					client.close();
    				}
    			//Updates the time of the connection for the blacklist stuff
  				long past = timeLast;
				...
				//Set timeout and start the response to the user		
